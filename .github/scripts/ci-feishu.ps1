...
function Get-FeishuLabels {
  return @{
    ...
    OtherEvent = (Get-UnicodeString 0x5176, 0x4ED6, 0x4E8B, 0x4EF6)         # other event
    EventTriggered = (Get-UnicodeString 0x4E8B, 0x4EF6, 0x5DF2, 0x89E6, 0x53D1)
    OpenRepo   = (Get-UnicodeString 0x6253, 0x5F00, 0x4ED3, 0x5E93)         # open repository
    PullRequest = (Get-UnicodeString 0x5408, 0x5E76, 0x8BF7, 0x6C42)        # pull request
    Issue      = (Get-UnicodeString 0x95EE, 0x9898)                         # issue
    Initiator  = (Get-UnicodeString 0x53D1, 0x8D77, 0x8005)                 # initiator
    ViewPullRequest = (Get-UnicodeString 0x67E5, 0x770B, 0x5408, 0x5E76, 0x8BF7, 0x6C42)
    ViewIssue  = (Get-UnicodeString 0x67E5, 0x770B, 0x95EE, 0x9898)
    Opened     = (Get-UnicodeString 0x5DF2, 0x521B, 0x5EFA)                 # created
    Closed     = (Get-UnicodeString 0x5DF2, 0x5173, 0x95ED)                 # closed
    Reopened   = (Get-UnicodeString 0x5DF2, 0x91CD, 0x65B0, 0x6253, 0x5F00)
    Edited     = (Get-UnicodeString 0x5DF2, 0x7F16, 0x8F91)                 # edited
    Synchronized = (Get-UnicodeString 0x5DF2, 0x540C, 0x6B65)              # synchronized
