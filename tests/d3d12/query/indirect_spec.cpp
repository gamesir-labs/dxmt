...
CreateDispatchSignature(ID3D12Device *device) {
  ...
  if (!device || FAILED(device->CreateCommandSignature(
...
enum class InvalidExecuteIndirectCase {
  ForeignSignature,
  ForeignArgumentBuffer,
  ForeignCountBuffer,
  ArgumentTexture,
  CountTexture,
  MisalignedArgumentOffset,
  MisalignedCountOffset,
  ArgumentRangeOutOfBounds,
  CountRangeOutOfBounds,
};

class InvalidExecuteIndirectSpec
    : public ::testing::Test,
      public ::testing::WithParamInterface<InvalidExecuteIndirectCase> {
protected:
...
TEST_P(InvalidExecuteIndirectSpec,
       RejectsInvalidArgumentsAndAllowsFreshListRecovery) {
  const D3D12_DISPATCH_ARGUMENTS command = {1, 1, 1};
  const UINT count_value = 1;
  auto local_signature = CreateDispatchSignature(context_.device());
  auto local_arguments = context_.CreateUploadBuffer(
      sizeof(command), &command, sizeof(command));
  auto local_count = context_.CreateUploadBuffer(
      sizeof(count_value), &count_value, sizeof(count_value));
  ASSERT_TRUE(local_signature);
  ASSERT_TRUE(local_arguments);
  ASSERT_TRUE(local_count);
  ComPtr<ID3D12CommandSignature> foreign_signature;
  ComPtr<ID3D12Resource> alternate_arguments;
  ComPtr<ID3D12Resource> alternate_count;
  ID3D12CommandSignature *signature = local_signature.get();
  ID3D12Resource *arguments = local_arguments.get();
  ...
  switch (GetParam()) {
  case InvalidExecuteIndirectCase::ForeignSignature:
    foreign_device = CreateIsolatedD3D12Device();
    ASSERT_TRUE(foreign_device);
    foreign_signature = CreateDispatchSignature(foreign_device.get());
    signature = foreign_signature.get();
    break;
  case InvalidExecuteIndirectCase::ForeignArgumentBuffer:
    foreign_device = CreateIsolatedD3D12Device();
    ASSERT_TRUE(foreign_device);
    ASSERT_EQ(foreign_context.Initialize(foreign_device.get()), S_OK);
    alternate_arguments = foreign_context.CreateUploadBuffer(
        sizeof(command), &command, sizeof(command));
    arguments = alternate_arguments.get();
    break;
  case InvalidExecuteIndirectCase::ForeignCountBuffer:
    foreign_device = CreateIsolatedD3D12Device();
    ASSERT_TRUE(foreign_device);
    ASSERT_EQ(foreign_context.Initialize(foreign_device.get()), S_OK);
    alternate_count = foreign_context.CreateUploadBuffer(
        sizeof(count_value), &count_value, sizeof(count_value));
    count = alternate_count.get();
    break;
  case InvalidExecuteIndirectCase::ArgumentTexture:
    alternate_arguments = context_.CreateTexture2D(
        1, 1, 1, DXGI_FORMAT_R8G8B8A8_UNORM, D3D12_RESOURCE_FLAG_NONE,
        D3D12_RESOURCE_STATE_COMMON);
    arguments = alternate_arguments.get();
    break;
  case InvalidExecuteIndirectCase::CountTexture:
    alternate_count = context_.CreateTexture2D(
        1, 1, 1, DXGI_FORMAT_R8G8B8A8_UNORM, D3D12_RESOURCE_FLAG_NONE,
        D3D12_RESOURCE_STATE_COMMON);
    count = alternate_count.get();
    break;
  case InvalidExecuteIndirectCase::MisalignedArgumentOffset:
    argument_offset = 2;
    break;
  case InvalidExecuteIndirectCase::MisalignedCountOffset:
    count = local_count.get();
    count_offset = 2;
    break;
  case InvalidExecuteIndirectCase::ArgumentRangeOutOfBounds:
    argument_offset = sizeof(UINT);
    break;
  case InvalidExecuteIndirectCase::CountRangeOutOfBounds:
    count = local_count.get();
    count_offset = sizeof(UINT);
    break;
  }
  ASSERT_NE(signature, nullptr);
  ASSERT_NE(arguments, nullptr);
  const bool requires_count =
      GetParam() == InvalidExecuteIndirectCase::ForeignCountBuffer ||
      GetParam() == InvalidExecuteIndirectCase::CountTexture ||
      GetParam() == InvalidExecuteIndirectCase::MisalignedCountOffset ||
      GetParam() == InvalidExecuteIndirectCase::CountRangeOutOfBounds;
  ASSERT_TRUE(!requires_count || count != nullptr);

  context_.list()->ExecuteIndirect(signature, 1, arguments, argument_offset,
                                   count, count_offset);
  ...
  ASSERT_EQ(context_.device()->CreateCommandList(
            ...
            S_OK);
  list->ExecuteIndirect(local_signature.get(), 1, local_arguments.get(), 0,
                        nullptr, 0);
  EXPECT_EQ(list->Close(), S_OK);
  EXPECT_EQ(context_.device()->GetDeviceRemovedReason(), S_OK);
}

std::string InvalidExecuteIndirectCaseName(
    const ::testing::TestParamInfo<InvalidExecuteIndirectCase> &info) {
  switch (info.param) {
  case InvalidExecuteIndirectCase::ForeignSignature:
    return "ForeignSignature";
  case InvalidExecuteIndirectCase::ForeignArgumentBuffer:
    return "ForeignArgumentBuffer";
  case InvalidExecuteIndirectCase::ForeignCountBuffer:
    return "ForeignCountBuffer";
  case InvalidExecuteIndirectCase::ArgumentTexture:
    return "ArgumentTexture";
  case InvalidExecuteIndirectCase::CountTexture:
    return "CountTexture";
  case InvalidExecuteIndirectCase::MisalignedArgumentOffset:
    return "MisalignedArgumentOffset";
  case InvalidExecuteIndirectCase::MisalignedCountOffset:
    return "MisalignedCountOffset";
  case InvalidExecuteIndirectCase::ArgumentRangeOutOfBounds:
    return "ArgumentRangeOutOfBounds";
  case InvalidExecuteIndirectCase::CountRangeOutOfBounds:
    return "CountRangeOutOfBounds";
  }
  return "Unknown";
}

INSTANTIATE_TEST_SUITE_P(
    Validation, InvalidExecuteIndirectSpec,
    ::testing::Values(
        InvalidExecuteIndirectCase::ForeignSignature,
        InvalidExecuteIndirectCase::ForeignArgumentBuffer,
        InvalidExecuteIndirectCase::ForeignCountBuffer,
        InvalidExecuteIndirectCase::ArgumentTexture,
        InvalidExecuteIndirectCase::CountTexture,
        InvalidExecuteIndirectCase::MisalignedArgumentOffset,
        InvalidExecuteIndirectCase::MisalignedCountOffset,
        InvalidExecuteIndirectCase::ArgumentRangeOutOfBounds,
        InvalidExecuteIndirectCase::CountRangeOutOfBounds),
    InvalidExecuteIndirectCaseName);
