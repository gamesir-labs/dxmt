...
enum class ForeignClearUavKind {
  Uint,
  Float,
};

class ForeignClearUavSpec
    : public ClearUavSpec,
      public ::testing::WithParamInterface<ForeignClearUavKind> {};

TEST_P(ForeignClearUavSpec, RejectsForeignResourceAndAllowsFreshListRecovery) {
  ...
  D3D12_UNORDERED_ACCESS_VIEW_DESC desc = {};
  desc.Format = GetParam() == ForeignClearUavKind::Uint
                    ? DXGI_FORMAT_R32_UINT
                    : DXGI_FORMAT_R32_FLOAT;
  desc.ViewDimension = D3D12_UAV_DIMENSION_BUFFER;
  desc.Buffer.NumElements = 4;
  const auto foreign_cpu =
      foreign_heap->GetCPUDescriptorHandleForHeapStart();
  foreign_device->CreateUnorderedAccessView(foreign_resource.get(), nullptr,
                                             &desc, foreign_cpu);
  constexpr std::array<UINT, 4> uint_values = {1, 2, 3, 4};
  constexpr std::array<float, 4> float_values = {1.0f, 2.0f, 3.0f, 4.0f};
  const auto foreign_gpu =
      foreign_heap->GetGPUDescriptorHandleForHeapStart();
  if (GetParam() == ForeignClearUavKind::Uint) {
    context_.list()->ClearUnorderedAccessViewUint(
        foreign_gpu, foreign_cpu, foreign_resource.get(), uint_values.data(),
        0, nullptr);
  } else {
    context_.list()->ClearUnorderedAccessViewFloat(
        foreign_gpu, foreign_cpu, foreign_resource.get(), float_values.data(),
        0, nullptr);
  }
  ...
  const auto local_cpu = local_heap->GetCPUDescriptorHandleForHeapStart();
  context_.device()->CreateUnorderedAccessView(local_resource.get(), nullptr,
                                               &desc, local_cpu);
  ...
  ASSERT_EQ(context_.device()->CreateCommandList(
            ...
            S_OK);
  ID3D12DescriptorHeap *heaps[] = {local_heap.get()};
  list->SetDescriptorHeaps(1, heaps);
  const auto local_gpu = local_heap->GetGPUDescriptorHandleForHeapStart();
  if (GetParam() == ForeignClearUavKind::Uint) {
    list->ClearUnorderedAccessViewUint(local_gpu, local_cpu,
                                       local_resource.get(), uint_values.data(),
                                       0, nullptr);
  } else {
    list->ClearUnorderedAccessViewFloat(
        local_gpu, local_cpu, local_resource.get(), float_values.data(), 0,
        nullptr);
  }
...
INSTANTIATE_TEST_SUITE_P(UintAndFloat, ForeignClearUavSpec,
                         ::testing::Values(ForeignClearUavKind::Uint,
                                           ForeignClearUavKind::Float));
