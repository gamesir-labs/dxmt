...
std::vector<CopyOffsetCase> BuildCopyOffsetCases() {
  ...
  const UINT64 anchors[] = {0, 1, 4, 8, 16, 32, 64, 128, 256, 512};
  for (const UINT64 size : sizes) {
    for (const UINT64 src : anchors) {
      for (const UINT64 dst : anchors) {
        if (src + size > kBuf || dst + size > kBuf)
          continue;
        cases.push_back({kBuf, src, dst, size});
      }
    }
  }
