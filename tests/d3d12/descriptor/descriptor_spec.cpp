...
TEST_F(D3D12DescriptorSpec, NullTypedUavReadReturnsZeroAndWriteIsDiscarded) {
  ...
  // Initialize probe with sentinels so discarded null-UAV stores cannot look
  // like success if the shader never runs.
  {
  ...
  // Null UAV loads return zero; stores are discarded so the second load stays 0.
  EXPECT_EQ(loaded, 0u);
  EXPECT_EQ(after_store, 0u);
  EXPECT_EQ(context_.device()->GetDeviceRemovedReason(), S_OK);
}

TEST_F(D3D12DescriptorSpec, NullTexture2dUavWriteIsDiscardedWhereDefined) {
  const auto shader = CompileShader(R"(
