...
// Public D3D11 non-array Texture1D UAV descriptor coverage. Two compatible
...
constexpr std::uint32_t kTexture1dUavDescCaseCount = 16;

const dxmt::test::LogicalCaseFamilyRegistration kTexture1dUavDescCases(
    "D3D11Texture1DSingleUnorderedAccessViewDescMatrixSpec."
    "RoundTrips16FormatMipSlices",
    "D3D11.UAV.Texture1D.Description.", kTexture1dUavDescCaseCount, 2,
    {dxmt::test::TestClass::Conformance,
     dxmt::test::ExecutionPath::Auto,
     {"11_0", "None", "Device",
      "CreateTexture1D,CreateUnorderedAccessView,"
...
const dxmt::test::TestCostRegistration kTexture1dUavDescCost(
    "D3D11Texture1DSingleUnorderedAccessViewDescMatrixSpec."
    "RoundTrips16FormatMipSlices",
    dxmt::test::kResourceTestCost);
...
struct Texture1dUavDescCase {
  UINT format_index;
  UINT mip_index;
  D3D11_UNORDERED_ACCESS_VIEW_DESC desc;
};

Texture1dUavDescCase CaseForLogical(std::uint32_t logical) {
  Texture1dUavDescCase test_case = {};
  test_case.format_index = logical & 1u;
  test_case.mip_index = logical >> 1u;
  test_case.desc.Format = kViewFormats[test_case.format_index];
  test_case.desc.ViewDimension = D3D11_UAV_DIMENSION_TEXTURE1D;
  test_case.desc.Texture1D.MipSlice = test_case.mip_index;
  return test_case;
}

bool Texture1dUavDescsEqual(const D3D11_UNORDERED_ACCESS_VIEW_DESC &actual,
...
class D3D11Texture1DSingleUnorderedAccessViewDescMatrixSpec
    : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(D3D11Texture1DSingleUnorderedAccessViewDescMatrixSpec,
       RoundTrips16FormatMipSlices) {
  std::vector<std::uint32_t> selected_cases;
  selected_cases.reserve(kTexture1dUavDescCaseCount);
  for (std::uint32_t logical = 0; logical < kTexture1dUavDescCaseCount;
       ++logical) {
    if (dxmt::test::LogicalCaseSelected(kTexture1dUavDescCases.family(),
                                        logical))
  ...
  RecordProperty("logical_case_prefix",
                 kTexture1dUavDescCases.family().case_id_prefix);
  for (const std::uint32_t logical : selected_cases) {
    const Texture1dUavDescCase test_case = CaseForLogical(logical);
    ...
    const bool desc_matches = Texture1dUavDescsEqual(actual, test_case.desc);
    ...
    const auto case_id =
        dxmt::test::LogicalCaseId(kTexture1dUavDescCases.family(), logical);
    ADD_FAILURE()
        << "LogicalCaseId: " << case_id << '\n'
        << "Class: "
        << dxmt::test::TestClassName(
               kTexture1dUavDescCases.family().traits.test_class)
        << '\n'
        << "Requirements: feature_level=11_0 shader_model=None queue=Device "
           "capability=CreateTexture1D,CreateUnorderedAccessView,"
           "UnorderedAccessViewGetDesc,ViewGetResource,DeviceChildGetDevice\n"
        << "ExecutionPath: "
        << dxmt::test::ExecutionPathName(
               kTexture1dUavDescCases.family().traits.execution_path)
        << '\n'
