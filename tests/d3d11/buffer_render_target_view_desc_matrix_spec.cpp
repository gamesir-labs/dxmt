...
// Public D3D11 buffer render-target-view descriptor coverage. Typed formats
...
constexpr std::uint32_t kBufferRtvDescCaseCount = 4096;

const dxmt::test::LogicalCaseFamilyRegistration kBufferRtvDescCases(
    "D3D11BufferRenderTargetViewDescMatrixSpec."
    "RoundTrips4096TypedElementRanges",
    "D3D11.RTV.Buffer.Description.", kBufferRtvDescCaseCount, 4,
    {dxmt::test::TestClass::Conformance,
     dxmt::test::ExecutionPath::Auto,
     {"11_0", "None", "Device",
      "CreateBuffer,CreateRenderTargetView,ID3D11RenderTargetViewGetDesc,"
...
const dxmt::test::TestCostRegistration
    kBufferRtvDescCost("D3D11BufferRenderTargetViewDescMatrixSpec."
                       "RoundTrips4096TypedElementRanges",
                       dxmt::test::kResourceTestCost);
...
BufferRtvDescCase CaseForLogical(std::uint32_t logical) {
  BufferRtvDescCase test_case = {};
  ...
  test_case.desc.ViewDimension = D3D11_RTV_DIMENSION_BUFFER;
...
bool BufferRtvDescsEqual(const D3D11_RENDER_TARGET_VIEW_DESC &actual,
...
class D3D11BufferRenderTargetViewDescMatrixSpec : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(D3D11BufferRenderTargetViewDescMatrixSpec,
       RoundTrips4096TypedElementRanges) {
  std::vector<std::uint32_t> selected_cases;
  selected_cases.reserve(kBufferRtvDescCaseCount);
  for (std::uint32_t logical = 0; logical < kBufferRtvDescCaseCount;
       ++logical) {
    if (dxmt::test::LogicalCaseSelected(kBufferRtvDescCases.family(), logical))
  ...
  RecordProperty("logical_case_prefix",
                 kBufferRtvDescCases.family().case_id_prefix);
  for (const std::uint32_t logical : selected_cases) {
    const BufferRtvDescCase test_case = CaseForLogical(logical);
    ...
    const bool desc_matches = BufferRtvDescsEqual(actual, test_case.desc);
    ...
    const auto case_id =
        dxmt::test::LogicalCaseId(kBufferRtvDescCases.family(), logical);
    ADD_FAILURE()
        << "LogicalCaseId: " << case_id << '\n'
        << "Class: "
        << dxmt::test::TestClassName(
               kBufferRtvDescCases.family().traits.test_class)
        << '\n'
        << "Requirements: feature_level=11_0 shader_model=None queue=Device "
           "capability=CreateBuffer,CreateRenderTargetView,"
           "RenderTargetViewGetDesc,ViewGetResource,DeviceChildGetDevice\n"
        << "ExecutionPath: "
        << dxmt::test::ExecutionPathName(
               kBufferRtvDescCases.family().traits.execution_path)
        << '\n'
...
TEST_F(D3D11BufferRenderTargetViewDescMatrixSpec,
  ...
  EXPECT_EQ(
      context_.device()->CreateRenderTargetView(buffer_.get(), &valid, nullptr),
      S_FALSE);

  D3D11_RENDER_TARGET_VIEW_DESC past_end = valid;
  past_end.Buffer.FirstElement = 32;
  past_end.Buffer.NumElements = 64;
  EXPECT_EQ(context_.device()->CreateRenderTargetView(buffer_.get(), &past_end,
                                                      nullptr),
            E_INVALIDARG);
  ...
  view = reinterpret_cast<ID3D11RenderTargetView *>(
      static_cast<std::uintptr_t>(1));
  EXPECT_EQ(
      context_.device()->CreateRenderTargetView(buffer_.get(), &empty, &view),
      E_INVALIDARG);
  EXPECT_EQ(view, nullptr);
  EXPECT_EQ(context_.device()->GetDeviceRemovedReason(), S_OK);
}
