...
// Public D3D11 non-array Texture2D SRV descriptor coverage. Four compatible
...
constexpr std::uint32_t kTexture2dSrvDescCaseCount = 144;

const dxmt::test::LogicalCaseFamilyRegistration kTexture2dSrvDescCases(
    "D3D11Texture2DSingleShaderResourceViewDescMatrixSpec."
    "RoundTrips144CompleteMipRanges",
    "D3D11.SRV.Texture2D.Description.", kTexture2dSrvDescCaseCount, 3,
...
const dxmt::test::TestCostRegistration kTexture2dSrvDescCost(
    "D3D11Texture2DSingleShaderResourceViewDescMatrixSpec."
    "RoundTrips144CompleteMipRanges",
    dxmt::test::kResourceTestCost);
...
struct Texture2dSrvDescCase {
  UINT format_index;
  UINT mip_range_index;
  D3D11_SHADER_RESOURCE_VIEW_DESC desc;
};

Texture2dSrvDescCase CaseForLogical(std::uint32_t logical) {
  Texture2dSrvDescCase test_case = {};
  ...
  for (UINT first = 0; first < kMipLevelCount; ++first) {
    ...
    if (range < count_at_first) {
      ...
      test_case.desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
      test_case.desc.Texture2D.MostDetailedMip = first;
      test_case.desc.Texture2D.MipLevels = range + 1u;
      return test_case;
    }
...
bool Texture2dSrvDescsEqual(const D3D11_SHADER_RESOURCE_VIEW_DESC &actual,
  ...
  return actual.Format == expected.Format &&
         ...
         actual.Texture2D.MostDetailedMip ==
             expected.Texture2D.MostDetailedMip &&
         actual.Texture2D.MipLevels == expected.Texture2D.MipLevels;
}

class D3D11Texture2DSingleShaderResourceViewDescMatrixSpec
    : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(D3D11Texture2DSingleShaderResourceViewDescMatrixSpec,
       RoundTrips144CompleteMipRanges) {
  std::vector<std::uint32_t> selected_cases;
  selected_cases.reserve(kTexture2dSrvDescCaseCount);
  for (std::uint32_t logical = 0; logical < kTexture2dSrvDescCaseCount;
       ++logical) {
    if (dxmt::test::LogicalCaseSelected(kTexture2dSrvDescCases.family(),
                                        logical))
  ...
  RecordProperty("logical_case_prefix",
                 kTexture2dSrvDescCases.family().case_id_prefix);
  for (const std::uint32_t logical : selected_cases) {
    const Texture2dSrvDescCase test_case = CaseForLogical(logical);
    ...
    const bool desc_matches = Texture2dSrvDescsEqual(actual, test_case.desc);
    ...
    const auto case_id =
        dxmt::test::LogicalCaseId(kTexture2dSrvDescCases.family(), logical);
    ADD_FAILURE()
        << "LogicalCaseId: " << case_id << '\n'
        << "Class: "
        << dxmt::test::TestClassName(
               kTexture2dSrvDescCases.family().traits.test_class)
        << '\n'
        ...
        << dxmt::test::ExecutionPathName(
               kTexture2dSrvDescCases.family().traits.execution_path)
        << '\n'
        ...
        << " mip_start=" << test_case.desc.Texture2D.MostDetailedMip
        << " mip_count=" << test_case.desc.Texture2D.MipLevels
        ...
        << " mip_start=" << actual.Texture2D.MostDetailedMip
        << " mip_count=" << actual.Texture2D.MipLevels
...
TEST_F(D3D11Texture2DSingleShaderResourceViewDescMatrixSpec,
  ...
  invalid_mip.Texture2D.MostDetailedMip = kMipLevelCount;
  EXPECT_EQ(context_.device()->CreateShaderResourceView(texture_.get(),
                                                        &invalid_mip, nullptr),
            E_INVALIDARG);

  D3D11_SHADER_RESOURCE_VIEW_DESC past_end = valid;
  past_end.Texture2D.MostDetailedMip = kMipLevelCount - 1u;
  past_end.Texture2D.MipLevels = 2;
  EXPECT_EQ(context_.device()->CreateShaderResourceView(texture_.get(),
                                                        &past_end, nullptr),
            E_INVALIDARG);

  D3D11_SHADER_RESOURCE_VIEW_DESC empty = valid;
  empty.Texture2D.MipLevels = 0;
