...
// Public D3D11 non-array Texture2DMS RTV descriptor coverage. Explicit and
...
constexpr std::uint32_t kTexture2dmsRtvDescCaseCount = 2;

const dxmt::test::LogicalCaseFamilyRegistration kTexture2dmsRtvDescCases(
    "D3D11Texture2DMSSingleRenderTargetViewDescMatrixSpec."
    "RoundTripsExplicitAndDefaultDescriptions",
    "D3D11.RTV.Texture2DMS.Description.", kTexture2dmsRtvDescCaseCount, 1,
    {dxmt::test::TestClass::Conformance,
     ...
     "and one live render-target view per selected logical case",
...
const dxmt::test::TestCostRegistration kTexture2dmsRtvDescCost(
    "D3D11Texture2DMSSingleRenderTargetViewDescMatrixSpec."
    "RoundTripsExplicitAndDefaultDescriptions",
    dxmt::test::kResourceTestCost);

D3D11_RENDER_TARGET_VIEW_DESC ExpectedDesc() {
...
class D3D11Texture2DMSSingleRenderTargetViewDescMatrixSpec
    : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(D3D11Texture2DMSSingleRenderTargetViewDescMatrixSpec,
       RoundTripsExplicitAndDefaultDescriptions) {
  ...
  const D3D11_RENDER_TARGET_VIEW_DESC expected = ExpectedDesc();
  ...
  for (const std::uint32_t logical : selected_cases) {
    ...
    const bool desc_matches = Texture2dmsRtvDescsEqual(actual, expected);
...
TEST_F(D3D11Texture2DMSSingleRenderTargetViewDescMatrixSpec,
  ...
  const D3D11_RENDER_TARGET_VIEW_DESC valid = ExpectedDesc();
  EXPECT_EQ(context_.device()->CreateRenderTargetView(texture_.get(), &valid,
                                                      nullptr),
            S_FALSE);

  D3D11_RENDER_TARGET_VIEW_DESC invalid = {};
  invalid.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
  invalid.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
