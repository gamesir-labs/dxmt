...
// Public D3D11 Texture2DMSArray DSV descriptor coverage. Read-only flags and
// array ranges form exactly 4096 distinct valid multisampled views.
...
constexpr std::uint32_t kTexture2dmsDsvDescCaseCount = 4096;

const dxmt::test::LogicalCaseFamilyRegistration kTexture2dmsDsvDescCases(
    "D3D11Texture2DMSDepthStencilViewDescMatrixSpec."
    "RoundTrips4096FlagArrayRanges",
    "D3D11.DSV.Texture2DMSArray.Description.", kTexture2dmsDsvDescCaseCount, 4,
    {dxmt::test::TestClass::Conformance,
     dxmt::test::ExecutionPath::Auto,
     {"11_0", "None", "Device",
      "CheckMultisampleQualityLevels,CreateTexture2D,CreateDepthStencilView,"
      "ID3D11DepthStencilViewGetDesc,ID3D11ViewGetResource,"
      "ID3D11DeviceChildGetDevice"},
     dxmt::test::kResourceTestCost,
     "one test-local D3D11 device, one 4x multisampled Texture2D array, and "
     "one live depth-stencil view per selected logical case",
     "create every combination of four depth/stencil read-only flag states, "
     "16 first array slices, and 64 array sizes, then query the complete "
     "public descriptor and relationships",
     "every view preserves its flags and multisampled array range and returns "
     "the source texture and creating device through public COM interfaces",
...
const dxmt::test::TestCostRegistration
    kTexture2dmsDsvDescCost("D3D11Texture2DMSDepthStencilViewDescMatrixSpec."
                            "RoundTrips4096FlagArrayRanges",
                            dxmt::test::kResourceTestCost);
...
Texture2dmsDsvDescCase CaseForLogical(std::uint32_t logical) {
  Texture2dmsDsvDescCase test_case = {};
  test_case.flag_index = logical & 3u;
  test_case.array_start_index = (logical >> 2u) & 15u;
  test_case.array_count_index = logical >> 6u;

  test_case.desc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
  test_case.desc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2DMSARRAY;
...
bool Texture2dmsDsvDescsEqual(const D3D11_DEPTH_STENCIL_VIEW_DESC &actual,
...
class D3D11Texture2DMSDepthStencilViewDescMatrixSpec : public ::testing::Test {
protected:
  void SetUp() override {
    ...
    ASSERT_EQ(context_.device()->CheckMultisampleQualityLevels(
                  DXGI_FORMAT_D24_UNORM_S8_UINT, 4, &quality_levels),
              S_OK);
...
TEST_F(D3D11Texture2DMSDepthStencilViewDescMatrixSpec,
       RoundTrips4096FlagArrayRanges) {
  std::vector<std::uint32_t> selected_cases;
  selected_cases.reserve(kTexture2dmsDsvDescCaseCount);
  for (std::uint32_t logical = 0; logical < kTexture2dmsDsvDescCaseCount;
       ++logical) {
    if (dxmt::test::LogicalCaseSelected(kTexture2dmsDsvDescCases.family(),
                                        logical))
  ...
  RecordProperty("logical_case_prefix",
                 kTexture2dmsDsvDescCases.family().case_id_prefix);
  for (const std::uint32_t logical : selected_cases) {
    const Texture2dmsDsvDescCase test_case = CaseForLogical(logical);
    ...
    const bool desc_matches = Texture2dmsDsvDescsEqual(actual, test_case.desc);
    ...
    const auto case_id =
        dxmt::test::LogicalCaseId(kTexture2dmsDsvDescCases.family(), logical);
    ADD_FAILURE()
        << "LogicalCaseId: " << case_id << '\n'
        << "Class: "
        << dxmt::test::TestClassName(
               kTexture2dmsDsvDescCases.family().traits.test_class)
        << '\n'
        << "Requirements: feature_level=11_0 shader_model=None queue=Device "
           "capability=CheckMultisampleQualityLevels,CreateTexture2D,"
           "CreateDepthStencilView,DepthStencilViewGetDesc,ViewGetResource,"
           "DeviceChildGetDevice\n"
        << "ExecutionPath: "
        << dxmt::test::ExecutionPathName(
               kTexture2dmsDsvDescCases.family().traits.execution_path)
        << '\n'
...
TEST_F(D3D11Texture2DMSDepthStencilViewDescMatrixSpec,
  ...
  ComPtr<ID3D11DepthStencilView> default_view;
  ASSERT_EQ(context_.device()->CreateDepthStencilView(texture_.get(), nullptr,
                                                      default_view.put()),
            S_OK);
  ASSERT_NE(default_view.get(), nullptr);
  D3D11_DEPTH_STENCIL_VIEW_DESC default_desc = {};
  default_view->GetDesc(&default_desc);
  EXPECT_EQ(default_desc.Format, DXGI_FORMAT_D24_UNORM_S8_UINT);
  EXPECT_EQ(default_desc.ViewDimension, D3D11_DSV_DIMENSION_TEXTURE2DMSARRAY);
  EXPECT_EQ(default_desc.Flags, 0u);
  EXPECT_EQ(default_desc.Texture2DMSArray.FirstArraySlice, 0u);
  EXPECT_EQ(default_desc.Texture2DMSArray.ArraySize, 79u);

  D3D11_TEXTURE2D_DESC typeless_desc = {};
  texture_->GetDesc(&typeless_desc);
  typeless_desc.ArraySize = 1;
  typeless_desc.Format = DXGI_FORMAT_R24G8_TYPELESS;
  ComPtr<ID3D11Texture2D> typeless_texture;
  ASSERT_EQ(context_.device()->CreateTexture2D(&typeless_desc, nullptr,
                                               typeless_texture.put()),
            S_OK);
  view = reinterpret_cast<ID3D11DepthStencilView *>(
      static_cast<std::uintptr_t>(1));
  EXPECT_EQ(context_.device()->CreateDepthStencilView(typeless_texture.get(),
                                                      nullptr, &view),
            E_INVALIDARG);
  EXPECT_EQ(view, nullptr);
  ...
  D3D11_DEPTH_STENCIL_VIEW_DESC past_end = valid;
  past_end.Texture2DMSArray.FirstArraySlice = 16;
  past_end.Texture2DMSArray.ArraySize = 64;
  EXPECT_EQ(context_.device()->CreateDepthStencilView(texture_.get(), &past_end,
                                                      nullptr),
            E_INVALIDARG);

  D3D11_DEPTH_STENCIL_VIEW_DESC empty = valid;
  empty.Texture2DMSArray.ArraySize = 0;
  view = reinterpret_cast<ID3D11DepthStencilView *>(
      static_cast<std::uintptr_t>(1));
  EXPECT_EQ(
      context_.device()->CreateDepthStencilView(texture_.get(), &empty, &view),
      E_INVALIDARG);
  EXPECT_EQ(view, nullptr);
  EXPECT_EQ(context_.device()->GetDeviceRemovedReason(), S_OK);
}
