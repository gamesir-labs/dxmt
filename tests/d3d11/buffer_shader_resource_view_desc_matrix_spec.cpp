...
// Public D3D11 buffer shader-resource-view descriptor coverage. Typed formats
...
constexpr std::uint32_t kBufferSrvDescCaseCount = 4096;

const dxmt::test::LogicalCaseFamilyRegistration kBufferSrvDescCases(
    "D3D11BufferShaderResourceViewDescMatrixSpec."
    "RoundTrips4096TypedElementRanges",
    "D3D11.SRV.Buffer.Description.", kBufferSrvDescCaseCount, 4,
    {dxmt::test::TestClass::Conformance,
     dxmt::test::ExecutionPath::Auto,
     {"11_0", "None", "Device",
      "CreateBuffer,CreateShaderResourceView,"
...
const dxmt::test::TestCostRegistration
    kBufferSrvDescCost("D3D11BufferShaderResourceViewDescMatrixSpec."
                       "RoundTrips4096TypedElementRanges",
                       dxmt::test::kResourceTestCost);
...
BufferSrvDescCase CaseForLogical(std::uint32_t logical) {
  BufferSrvDescCase test_case = {};
  ...
  test_case.desc.ViewDimension = D3D11_SRV_DIMENSION_BUFFER;
...
bool BufferSrvDescsEqual(const D3D11_SHADER_RESOURCE_VIEW_DESC &actual,
...
class D3D11BufferShaderResourceViewDescMatrixSpec : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(D3D11BufferShaderResourceViewDescMatrixSpec,
       RoundTrips4096TypedElementRanges) {
  std::vector<std::uint32_t> selected_cases;
  selected_cases.reserve(kBufferSrvDescCaseCount);
  for (std::uint32_t logical = 0; logical < kBufferSrvDescCaseCount;
       ++logical) {
    if (dxmt::test::LogicalCaseSelected(kBufferSrvDescCases.family(), logical))
  ...
  RecordProperty("logical_case_prefix",
                 kBufferSrvDescCases.family().case_id_prefix);
  for (const std::uint32_t logical : selected_cases) {
    const BufferSrvDescCase test_case = CaseForLogical(logical);
    ...
    const bool desc_matches = BufferSrvDescsEqual(actual, test_case.desc);
    ...
    const auto case_id =
        dxmt::test::LogicalCaseId(kBufferSrvDescCases.family(), logical);
    ADD_FAILURE()
        << "LogicalCaseId: " << case_id << '\n'
        << "Class: "
        << dxmt::test::TestClassName(
               kBufferSrvDescCases.family().traits.test_class)
        << '\n'
        << "Requirements: feature_level=11_0 shader_model=None queue=Device "
           "capability=CreateBuffer,CreateShaderResourceView,"
           "ShaderResourceViewGetDesc,ViewGetResource,DeviceChildGetDevice\n"
        << "ExecutionPath: "
        << dxmt::test::ExecutionPathName(
               kBufferSrvDescCases.family().traits.execution_path)
        << '\n'
...
TEST_F(D3D11BufferShaderResourceViewDescMatrixSpec,
  ...
  EXPECT_EQ(context_.device()->CreateShaderResourceView(buffer_.get(), &valid,
                                                        nullptr),
            S_FALSE);
  ...
  EXPECT_EQ(context_.device()->CreateShaderResourceView(buffer_.get(),
                                                        &past_end, nullptr),
            E_INVALIDARG);
  ...
  view = reinterpret_cast<ID3D11ShaderResourceView *>(
      static_cast<std::uintptr_t>(1));
  EXPECT_EQ(
      context_.device()->CreateShaderResourceView(buffer_.get(), &empty, &view),
      E_INVALIDARG);
  EXPECT_EQ(view, nullptr);
  EXPECT_EQ(context_.device()->GetDeviceRemovedReason(), S_OK);
}
