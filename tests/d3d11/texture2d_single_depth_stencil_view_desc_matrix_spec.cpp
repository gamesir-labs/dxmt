...
// Public D3D11 non-array Texture2D DSV descriptor coverage. Four read-only
...
constexpr std::uint32_t kTexture2dDsvDescCaseCount = 32;

const dxmt::test::LogicalCaseFamilyRegistration kTexture2dDsvDescCases(
    "D3D11Texture2DSingleDepthStencilViewDescMatrixSpec."
    "RoundTrips32FlagMipSlices",
    "D3D11.DSV.Texture2D.Description.", kTexture2dDsvDescCaseCount, 2,
    {dxmt::test::TestClass::Conformance,
     dxmt::test::ExecutionPath::Auto,
     {"11_0", "None", "Device",
      "CreateTexture2D,CreateDepthStencilView,ID3D11DepthStencilViewGetDesc,"
...
const dxmt::test::TestCostRegistration
    kTexture2dDsvDescCost("D3D11Texture2DSingleDepthStencilViewDescMatrixSpec."
                          "RoundTrips32FlagMipSlices",
                          dxmt::test::kResourceTestCost);
...
struct Texture2dDsvDescCase {
  UINT flag_index;
  UINT mip_index;
  D3D11_DEPTH_STENCIL_VIEW_DESC desc;
};

Texture2dDsvDescCase CaseForLogical(std::uint32_t logical) {
  Texture2dDsvDescCase test_case = {};
  test_case.flag_index = logical & 3u;
  test_case.mip_index = logical >> 2u;
  test_case.desc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
  test_case.desc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
  test_case.desc.Flags = kViewFlags[test_case.flag_index];
  test_case.desc.Texture2D.MipSlice = test_case.mip_index;
  return test_case;
}

bool Texture2dDsvDescsEqual(const D3D11_DEPTH_STENCIL_VIEW_DESC &actual,
...
class D3D11Texture2DSingleDepthStencilViewDescMatrixSpec
    : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(D3D11Texture2DSingleDepthStencilViewDescMatrixSpec,
       RoundTrips32FlagMipSlices) {
  std::vector<std::uint32_t> selected_cases;
  selected_cases.reserve(kTexture2dDsvDescCaseCount);
  for (std::uint32_t logical = 0; logical < kTexture2dDsvDescCaseCount;
       ++logical) {
    if (dxmt::test::LogicalCaseSelected(kTexture2dDsvDescCases.family(),
                                        logical))
  ...
  RecordProperty("logical_case_prefix",
                 kTexture2dDsvDescCases.family().case_id_prefix);
  for (const std::uint32_t logical : selected_cases) {
    const Texture2dDsvDescCase test_case = CaseForLogical(logical);
    ...
    const bool desc_matches = Texture2dDsvDescsEqual(actual, test_case.desc);
    ...
    const auto case_id =
        dxmt::test::LogicalCaseId(kTexture2dDsvDescCases.family(), logical);
    ADD_FAILURE()
        << "LogicalCaseId: " << case_id << '\n'
        << "Class: "
        << dxmt::test::TestClassName(
               kTexture2dDsvDescCases.family().traits.test_class)
        << '\n'
        << "Requirements: feature_level=11_0 shader_model=None queue=Device "
           "capability=CreateTexture2D,CreateDepthStencilView,"
           "DepthStencilViewGetDesc,ViewGetResource,DeviceChildGetDevice\n"
        << "ExecutionPath: "
        << dxmt::test::ExecutionPathName(
               kTexture2dDsvDescCases.family().traits.execution_path)
        << '\n'
        ...
        << " flags=" << actual.Flags << " mip=" << actual.Texture2D.MipSlice
