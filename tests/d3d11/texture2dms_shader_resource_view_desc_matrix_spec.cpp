...
// Public D3D11 Texture2DMSArray SRV descriptor coverage. First array slices
...
constexpr std::uint32_t kTexture2dmsSrvDescCaseCount = 4096;

const dxmt::test::LogicalCaseFamilyRegistration kTexture2dmsSrvDescCases(
    "D3D11Texture2DMSShaderResourceViewDescMatrixSpec."
    "RoundTrips4096ArrayRanges",
    "D3D11.SRV.Texture2DMSArray.Description.", kTexture2dmsSrvDescCaseCount, 4,
    {dxmt::test::TestClass::Conformance,
     ...
     "one live shader-resource view per selected logical case",
...
const dxmt::test::TestCostRegistration
    kTexture2dmsSrvDescCost("D3D11Texture2DMSShaderResourceViewDescMatrixSpec."
                            "RoundTrips4096ArrayRanges",
                            dxmt::test::kResourceTestCost);

struct Texture2dmsSrvDescCase {
  UINT array_start_index;
  UINT array_count_index;
  D3D11_SHADER_RESOURCE_VIEW_DESC desc;
};

Texture2dmsSrvDescCase CaseForLogical(std::uint32_t logical) {
  Texture2dmsSrvDescCase test_case = {};
  test_case.array_start_index = logical & 63u;
  test_case.array_count_index = logical >> 6u;

  test_case.desc.Format = DXGI_FORMAT_R32_FLOAT;
  test_case.desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2DMSARRAY;
...
class D3D11Texture2DMSShaderResourceViewDescMatrixSpec
    : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(D3D11Texture2DMSShaderResourceViewDescMatrixSpec,
       RoundTrips4096ArrayRanges) {
  ...
  for (const std::uint32_t logical : selected_cases) {
    const Texture2dmsSrvDescCase test_case = CaseForLogical(logical);
    ...
    const bool desc_matches = Texture2dmsSrvDescsEqual(actual, test_case.desc);
...
TEST_F(D3D11Texture2DMSShaderResourceViewDescMatrixSpec,
  ...
  ComPtr<ID3D11ShaderResourceView> default_view;
  ASSERT_EQ(context_.device()->CreateShaderResourceView(texture_.get(), nullptr,
                                                        default_view.put()),
            S_OK);
  ASSERT_NE(default_view.get(), nullptr);
  D3D11_SHADER_RESOURCE_VIEW_DESC default_desc = {};
  default_view->GetDesc(&default_desc);
  EXPECT_EQ(default_desc.Format, DXGI_FORMAT_R32_FLOAT);
  EXPECT_EQ(default_desc.ViewDimension, D3D11_SRV_DIMENSION_TEXTURE2DMSARRAY);
