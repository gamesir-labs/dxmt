...
截至第三轮新增 `optional_command_contract_spec.cpp`，已覆盖 raytracing prebuild 输出清零、Build/Emit/Copy/SetPipelineState1、VRS image、Protected Session v1/type query、state-object growth 参数校验和 AtomicCopy UINT64；与已有 optional feature gate 合计执行 23 个 case。仍需补 sampler-feedback descriptor 的 inert oracle，以及带真实 protected session/state object 的正向参数路径（只有能力启用后才可成为 must-pass）。
