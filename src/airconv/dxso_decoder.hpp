...
// Result shift is sign-magnitude (bit 27 = sign, bits 24..26 = mag),
// per DXVK src/dxso/dxso_decoder.cpp; range is -8..+7.
...
walk_dxso_shader(const uint32_t *byte_code, uint32_t dword_count, const DxsoHeader &header) {
  ...
  while (it.next(ins)) {
    ...
    for (uint32_t i = 0; i < ins.src_count; ++i) {
      if (ins.src[i].has_relative && ins.src[i].base.type == DxsoRegisterType::Const)
