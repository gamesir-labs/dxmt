...
/* PS-only specialisation: D3D9 fixed fog blend. When present, the
   compiled PS (ps < 3_0 only; ps_3_0 computes fog itself per spec)
   emits rgb = mix(fog_color, rgb, saturate(fog)) with the factor from
   the FOG0 varying (VS oFog). The fog colour is read at runtime from
   the bool-constant buffer tail (buffer(2), float4 at byte offset 16)
   so it never enters the variant key. The host gates the variant on
   FOGENABLE + table mode NONE + vs.metadata.writes_fog: a VS that
   writes no oFog means factor 1.0 and an identity lerp. Both wined3d
   and DXVK bake the same blend into their generated pixel shaders. */
