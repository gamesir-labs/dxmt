...
static cl::opt<std::string> DxsoFfp(
    "dxso-ffp",
    cl::desc(
        "Compile a generated fixed-function shader instead of the input bytecode: vs, vs-t "
        "(pre-transformed position) or ps. The bytecode input still selects the pipeline stage plumbing."
    ),
    cl::init("")
);
...
int main(int argc, char **argv) {
  ...
  if (InputIsDxso) {
    ...
    if (DxsoFfp.getNumOccurrences()) {
      DXSO_SHADER_FFP_KEY_DATA key{};
      key.type = DXSO_SHADER_FFP_KEY;
      key.kind = DxsoFfp == "ps" ? 1u : 0u;
      key.has_diffuse = 1;
      // Canonical XYZ|DIFFUSE layout: FLOAT3 position at 0, UBYTE4N
      // diffuse at 12 (format ids mirror d3d9_device.cpp's map).
      DXSO_IA_INPUT_ELEMENT ffp_elements[2] = {
          {0u, 0u, 0u, 30u, 0u, 0u},
          {1u, 0u, 12u, 9u, 0u, 0u},
      };
      DXSO_SHADER_IA_INPUT_LAYOUT_DATA ffp_layout{};
      ffp_layout.type = DXSO_SHADER_IA_INPUT_LAYOUT;
      ffp_layout.num_elements = 2;
      ffp_layout.slot_mask = 1;
      ffp_layout.position_transformed = DxsoFfp == "vs-t" ? 1 : 0;
      ffp_layout.elements = ffp_elements;
      dxmt::compile_ffp(
          &key, &ffp_layout, /*ps_args=*/nullptr, /*ps_fog_mode=*/-1, /*ps_fog_coord_w=*/false, "shader_main", Context,
          M
      );
      ...
      dxmt::compile_dxso(
          ...
          /*vs_inject_point_size=*/false,
          /*ps_fog_mode=*/DxsoFogMode,
          /*ps_fog_coord_w=*/false, "shader_main", Context, M
      );
      dxmt::dxso_shader_destroy(dxso);
    }
  } else {
