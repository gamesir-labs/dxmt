...
// single shared flat typed argument-buffer array, stride 3 entries/descriptor
// ([handle, cube_handle, lod_bias]), indexed by absolute heap slot. The TEXTURE
// mirror keeps slot 30 (kArgumentBufferBindIndex), likewise a typed flat array,
// stride 2 ([handle, meta]). Slot 31 is intentionally NOT used (Metal guarantees
// buffer indices 0..30 only). See BINDLESS-ABI.md / sub-step ②③.
