...
static ReplayRttiCounters &replayRttiCounters() {
  static thread_local ReplayRttiCounters c;
  return c;
}
static bool ReplayBreakdownEnabled() {
  static const bool on = D3D12DiagEnabledEnv("DXMT_DIAG_REPLAY_BREAKDOWN");
  return on;
}
...
static bool DescAccessCacheEnabled() {
...
static void ResolveQueryDataToCpuBufferStatic(
...
static bool ReplaceDescriptorMirrorResidencyTargetForEncode(
...
static bool FillBindlessTextureBufferMirrorSlotForStage(
    ArgumentEncodingContext &enc, DescriptorHeapMirror &mirror, UINT slot,
...
struct CommandQueueResourceStates {
  ~CommandQueueResourceStates() {
    std::lock_guard lock(g_resource_states_mutex);
    auto entry = g_resource_states_by_device.find(device_key);
    if (entry != g_resource_states_by_device.end() && entry->second.expired())
      g_resource_states_by_device.erase(entry);
  }

  IMTLD3D12Device *device_key = nullptr;
...
private:
  ...
  struct ReplayBindingGenerationKey {
    uint64_t graphics = 0;
    dxmt::DescriptorContentRevision descriptor_content = {};
  ...
  struct ReplayBindingGenerationKeyHash {
    size_t operator()(const ReplayBindingGenerationKey &key) const {
      ...
      hash ^= std::hash<uint64_t>{}(key.descriptor_content.epoch) +
              0x9e3779b97f4a7c15ull + (hash << 6) + (hash >> 2);
      hash ^= std::hash<uint64_t>{}(key.descriptor_content.sequence) +
              0x9e3779b97f4a7c15ull + (hash << 6) + (hash >> 2);
      return hash;
    }
  };
  ...
  struct ReplayBlitPassCommand {
    std::function<void(ArgumentEncodingContext &)> encode;
    uint64_t d3d_sequence = 0;
  };
  ...
  struct ResourceAccessBarrierKeyHash {
    size_t operator()(const ResourceAccessBarrierKey &key) const {
      ...
      hash ^= (size_t(key.slice) + 0x9e3779b97f4a7c15ull + (hash << 6) +
               (hash >> 2));
      hash ^= (size_t(key.texture) + 0x9e3779b97f4a7c15ull + (hash << 6) +
               (hash >> 2));
      return hash;
    }
  };
  ...
  static constexpr size_t kResourceAccessBarrierLinearEntryLimit = 32;
  ...
  static void
  MergeResourceAccessBarrierEntry(ResourceAccessBarrierEntry &dst,
  ...
  static bool
  CanDeferResourceBarriersIntoComputeBatch(
      const ReplayState &state, const ResourceAccessBarrierBatch &batch) {
  ...
  static bool HasPendingBlitBatch(const ReplayState &state) {
  ...
  static ReplayGraphicsPassPlan
  BuildReplayGraphicsPassPlan(
  ...
  static bool HasPendingReplayWork(const ReplayState &state) {
  ...
  template <PipelineStage stage>
  static void EncodeResourceAccessBarrierEntriesForStage(
  ...
  static void EncodeResourceAccessBarrierEntries(
      ArgumentEncodingContext &enc,
      std::vector<ResourceAccessBarrierEntry> &entries) {
  ...
  static void EncodeRenderResourceAccessBarrierEntries(
      ArgumentEncodingContext &enc,
      std::vector<ResourceAccessBarrierEntry> &entries) {
  ...
  static void EncodeInlineRenderResourceBarrierEntries(
      ArgumentEncodingContext &enc,
      std::vector<ResourceAccessBarrierEntry> &entries) {
  ...
  bool ReplayBlitBatchHasHazard(const ReplayBlitBatch &batch,
                                ...
                                std::initializer_list<ID3D12Resource *> writes) const {
  ...
  void EmitTimestampResolve(CommandChunk *chunk, PendingTimestampResolve resolve) {
    ...
    if (!retained_queries.empty())
      chunk->addCompletionCallback(
          [queries = std::move(retained_queries)]() mutable {
            queries.clear();
          });
  }
  ...
  SliceResolveSnapshot(const QueryResolveSnapshot &snapshot, UINT offset,
    ...
    if (offset > snapshot.entries.size() ||
      ...
      return slice;
    slice.entries.insert(slice.entries.end(),
                         snapshot.entries.begin() + offset,
  ...
  bool DeferCpuQueryResolveToResource(const ResolveQueryDataRecord &record,
    ...
    const uintptr_t command_list_identity = record.command_list_identity;
    ...
    dst->AddPendingCpuQueryResolve(
        ...
        [command_list_identity, query_heap = std::move(query_heap), type,
         start_index, query_count, timestamp,
         dst_identity, dst_buffer_offset, queue_id,
         snapshot = std::move(snapshot)](Resource *resource) mutable {
          if (timestamp) {
          ...
          ResolveQueryDataToCpuBufferStatic(
              reinterpret_cast<const void *>(command_list_identity),
              query_heap.ptr(), type, start_index,
              query_count, resource, dst_identity, dst_buffer_offset,
              snapshot, "cpu-deferred-map", queue_id);
        });
    if (timestamp) {
  ...
  void QueueCpuQueryFallback(ReplayState &state,
    ...
    if (DeferCpuQueryResolveToResource(record, snapshot, byte_count)) {
  ...
  void ReplayCommandRecords(const std::vector<CommandRecord> &records,
    ...
    auto queue_compiled_graphics_packet =
        [&](const CompiledGraphicsPacket &packet)
          ...
          attachment_state.queue_type = state.queue_type;
          attachment_state.touched_resources = state.touched_resources;
          attachment_state.touched_resources_set = state.touched_resources_set;
          attachment_state.render_targets = packet.render_state.render_targets;
          attachment_state.depth_stencil = packet.render_state.depth_stencil;
          auto attachments =
              BuildRenderPassAttachments(attachment_state,
          ...
          if (packet.draw) {
            ...
            replay_packet.vertex_count = packet.draw->vertex_count_per_instance;
            ...
            replay_packet.index_offset =
                ...
                packet.draw_indexed->start_index_location *
                    GetIndexSize(packet.input_assembler.index_buffer->Format);
            replay_packet.index_count =
                packet.draw_indexed->index_count_per_instance;
            replay_packet.start_index =
                packet.draw_indexed->start_index_location;
            replay_packet.instance_count = packet.draw_indexed->instance_count;
            replay_packet.base_vertex =
  ...
  void RecordReplayResourceAccess(CommandChunk *chunk, ReplayState &state,
    ...
    if (desired != D3D12_RESOURCE_STATE_COMMON &&
      ...
      if (sit != state.steady_read_states.end() &&
  ...
  static void MergeResourceAccessBarrierBatch(ResourceAccessBarrierBatch &dst,
  ...
  void ReplayResolveQueryData(CommandChunk *chunk, ReplayState &state,
    ...
    if (D3D12TimestampGpuResolveEnabled() &&
      ...
      for (UINT i = 0; i < record.query_count;) {
        ...
        if (gpu_run) {
          ...
          QueueCpuQueryFallback(state, run_record, std::move(run_snapshot),
                                run_bytes,
    ...
    QueueCpuQueryFallback(state, record, std::move(snapshot),
  ...
  BindingPlan BuildBindingPlan(RootSignature *root, PipelineState &pipeline,
                               bool compute) {
    BindingPlan plan;
  ...
  D3D12_GPU_VIRTUAL_ADDRESS GetRootBufferAddressForFingerprint(
      const ReplayState &state, bool compute, UINT root_index,
      DescriptorRecordType type) const {
    const auto &map =
        type == DescriptorRecordType::ConstantBufferView
            ? (compute ? state.compute_cbv_roots : state.graphics_cbv_roots)
            : type == DescriptorRecordType::ShaderResourceView
      ...
      if (entry.range_type == D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER &&
        ...
        for (UINT local = 0;
          ...
          const auto slot_descriptor = GetBoundDescriptorRecordInRangeFromHeap(
              ...
              entry.heap_type);
          if (slot_descriptor)
            VerifyBindlessMirrorSamplerDescriptor(enc, *slot_descriptor,
                                                  want_stage, &argument);
        }
      }
      if (BindlessMirrorDiagEnabled() &&
  ...
  void CaptureGraphicsVertexBuffers(GraphicsBindingSnapshot &snapshot,
                                    const ReplayState &state,
          ...
          if (payload) {
            for (uint32_t pair = 0; pair < window->texture_field_pairs; pair++) {
              const uint64_t pair_base =
                  uint64_t(pair) * dxmt::kMirrorTextureQwords *
                  dxmt::kBindlessMirrorCapacity;
              dst[pair_base + dst_slot] = payload->handle;
              dst[pair_base + dxmt::kBindlessMirrorCapacity + dst_slot] =
                  payload->metadata;
            }
          }
        }
  ...
  struct BindlessBufferTableSnapshotStorage {
    std::array<ConstantBufferBindingSnapshot, 14 * kStages> constant_buffers =
        {};
    std::array<ShaderResourceBindingSnapshot, kSRVBindings * kStages>
        resources = {};

    BindlessBufferTableSnapshot view() const {
      return BindlessBufferTableSnapshot{
          constant_buffers.data(), resources.data()};
    }
  };
  ...
  void BindRootConstantsSnapshot(ArgumentEncodingContext &enc,
    ...
    if (!entry.constants.empty()) {
      std::memcpy(constants.mapped, entry.constants.data(),
                  std::min<uint64_t>(uint64_t(entry.constants.size()) *
                                         sizeof(UINT),
                                     constants.length));
    }
    if (constants.needs_flush)
  ...
  FindCompiledRootTable(const std::vector<CompiledCommandRootDescriptorTable> &tables,
  ...
  static uint64_t AllocateArgumentBuffer(uint64_t &cursor, uint64_t size) {
    ...
    const auto allocation_size = std::max<uint64_t>(size, 8);
  ...
  static uint64_t AdvanceArgumentBufferEstimate(uint64_t cursor,
                                                uint64_t size) {
  ...
  static uint64_t EstimateGraphicsArgumentBufferSize(PipelineState &pipeline,
    ...
    if (const auto *graphics = pipeline.GetGraphicsState()) {
      ...
      if (slot_mask)
        size = AdvanceArgumentBufferEstimate(
            size, uint64_t(__builtin_popcount(slot_mask)) * 16u);
    }
    ...
    for (const auto &shader : pipeline.GetDxilShaders()) {
      if (shader.stage == PipelineShaderStage::Vertex ||
        ...
        if (native)
          continue;
        const auto shader_size = EstimateShaderArgumentBufferSize(shader);
        if (shader_size)
  ...
  static void EncodeDynamicRenderState(
    ...
    auto scissor_values_equal = [](const std::vector<WMTScissorRect> &lhs,
      ...
      return lhs.size() == rhs.size() &&
             (!lhs.size() ||
              std::memcmp(lhs.data(), rhs.data(),
                          lhs.size() * sizeof(WMTScissorRect)) == 0);
    };
    auto *render_encoder = enc.currentRenderEncoder();
    ...
    if (!cache.viewports_valid ||
      ...
      auto &viewport_cmd =
          enc.encodeRenderCommand<wmtcmd_render_setviewports>();
  ...
  static Rc<VisibilityResultQuery> BeginReplayDrawVisibilityQuery(
      ArgumentEncodingContext &enc, Rc<VisibilityResultQuery> &visibility_query) {
  ...
  static void EndReplayDrawVisibilityQuery(
      ArgumentEncodingContext &enc,
      Rc<VisibilityResultQuery> &active_visibility_query) {
  ...
  void StopSubmissionWorker() {
    std::deque<PendingOperation> cancelled;
    {
      ...
      if (!submission_worker_stopping_) {
        // Operations before the first unresolved queue wait may still be
        // drained. The wait and everything ordered after it cannot make
        // progress during destruction, so cancel that suffix and put Stop at
        // the now-reachable tail.
        auto first_wait = std::find_if(
            pending_operations_.begin(), pending_operations_.end(),
            [](const PendingOperation &operation) {
              if (operation.type != PendingOperationType::Wait)
                return false;
              if ((operation.wait_completion &&
                   operation.wait_completion->load(
                       std::memory_order_acquire)) ||
                  operation.fence->GetCompletedValue() >= operation.value)
                return false;
              FenceGpuSignal signal = {};
              return operation.fence->TryResolveGpuWait(operation.value,
                                                        signal) !=
                     FenceGpuWaitStatus::Resolved;
            });
        while (first_wait != pending_operations_.end()) {
          cancelled.push_back(std::move(*first_wait));
          first_wait = pending_operations_.erase(first_wait);
        }
    ...
    for (;;) {
      {
        ...
        submission_wake_state_->condition.wait(lock, [this]() {
          if (pending_operations_.empty())
            return false;
          if (!submission_worker_waiting_for_wait_)
            return true;
          const auto &front = pending_operations_.front();
          return front.type == PendingOperationType::Wait &&
        ...
        if (front.type == PendingOperationType::Wait &&
          ...
          if (gpu_wait_status == FenceGpuWaitStatus::Resolved) {
            ...
            if (!front.wait_callback_armed) {
              ...
              wait_completion = front.wait_completion;
              ...
              // finishes; the callback itself runs from fence-owned storage.
...
AcquireDxmtQueueSubmissionGuard(IMTLD3D12Device *device) {
  auto states = GetDeviceResourceStates(device);
  return DxmtQueueSubmissionGuard{
      std::static_pointer_cast<void>(states),
      std::unique_lock<std::mutex>(states->dxmt_queue_mutex)};
}
