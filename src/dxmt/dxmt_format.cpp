...
static uint32_t
DXGIFormatTypeGroup(uint32_t format) {
  switch (format) {
...
bool
AreDXGIFormatsInSameTypeGroup(uint32_t lhs, uint32_t rhs) {
  return lhs == rhs || DXGIFormatTypeGroup(lhs) == DXGIFormatTypeGroup(rhs);
}
