...
WINEMETAL_API void
MTLCommandBuffer_registerResource(obj_handle_t cmdbuf,
...
WINEMETAL_API uint64_t
DispatchData_copy(obj_handle_t data, void *bytes, uint64_t capacity) {
  struct unixcall_dispatch_data_copy params;
  params.data = data;
  WMT_MEMPTR_SET(params.bytes, bytes);
  params.capacity = capacity;
  params.ret_length = 0;
  UNIX_CALL(155, &params);
  return params.ret_length;
}
...
WINEMETAL_API obj_handle_t
MTL4CounterHeap_newTimestampHeap(obj_handle_t device, uint64_t count, obj_handle_t *err_out) {
  struct unixcall_mtl4counterheap_newtimestampheap params;
  params.device = device;
  params.count = count;
  params.ret_error = 0;
  params.ret_heap = 0;
  ...
  if (err_out)
    *err_out = params.ret_error;
  return params.ret_heap;
}

WINEMETAL_API void
MTL4CounterHeap_resolveCounterRange(
    obj_handle_t heap, uint64_t start, uint64_t len, void *data_out, uint64_t data_length
) {
  struct unixcall_mtl4counterheap_resolvecounterrange params;
  params.heap = heap;
  params.start = start;
  params.len = len;
  params.data_length = data_length;
...
WINEMETAL_API void
MTL4TimestampContext_writeTimestamp(obj_handle_t context, obj_handle_t cmdbuf, obj_handle_t heap, uint64_t index) {
  struct unixcall_mtl4timestampcontext_writetimestamp params;
  params.context = context;
  params.mtl_command_buffer = cmdbuf;
  params.heap = heap;
  params.index = index;
...
WINEMETAL_API uint64_t
MTLDevice_sizeOfTimestampHeapEntry(obj_handle_t device) {
  struct unixcall_generic_obj_uint64_ret params;
  params.handle = device;
  params.ret = 0;
...
WINEMETAL_API obj_handle_t
MTL4TimestampContext_create(obj_handle_t device) {
  struct unixcall_generic_obj_obj_ret params;
  params.handle = device;
  params.ret = 0;
