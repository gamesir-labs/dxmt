...
@implementation WMT4CacheReader
...
@interface WMT4CacheWriter () {
  sqlite3 *_db;
  sqlite3_stmt *_stmt;
}
@end

@implementation WMT4CacheWriter
...
- (instancetype)initWithPath:(NSString *)path version:(uint64_t)version {
  if ((self = [super init])) {
    ...
    if (sqlite3_exec(_db, sqlCreate.UTF8String, NULL, NULL, &errMsg) != SQLITE_OK) {
      NSLog(@"[WMT4CacheWriter] Failed to create table: %s", errMsg);
      sqlite3_free(errMsg);
    }
...
- (void)set:(NSData *)key value:(dispatch_data_t)value {
  ...
  if (sqlite3_step(_stmt) != SQLITE_DONE) {
    NSLog(@"[WMT4CacheWriter] Failed to insert: %s", sqlite3_errmsg(_db));
  }
...
_WMT4CacheWriter_set(void *obj) {
  ...
  NSData *key =
      [[NSData alloc] initWithBytesNoCopy:(void *)params->key.ptr length:params->key_length freeWhenDone:false];
  WMT4CacheWriter *writer = (WMT4CacheWriter *)params->cache;
