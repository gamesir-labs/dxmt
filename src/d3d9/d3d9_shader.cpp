...
WMT::Function
MTLD3D9PixelShaderModule::compileVariant(
    uint32_t alpha_test_func, uint32_t alpha_test_ref, const uint8_t samp_kinds[16], bool point_sprite,
    const ::DXSO_SHADER_PS_BUMP_ENV_DATA *bump_env, int fog_mode, bool dual_source
) {
  // FNV-1a over the (alpha-test, sampler-kinds, point-sprite, bump-env,
  // fog-mode) tuple. Alpha args use 8 bits each; kinds are also 8 bits each;
  ...
  // Fog folds in the mode only (none/vertex/linear/exp/exp2, four real
  // variants): the colour and the table params are runtime constants
  // read from the bool-buffer tail, never part of the key.
  ...
  // D3DTSS_BUMPENV* for TexBem); PS_FOG (if active, carries the fog mode: vertex or a table mode).
  ...
  if (fog_mode >= 0) {
    ...
    fog_arg.mode = (uint32_t)fog_mode;
    *tail_next = &fog_arg;
  }

  // The PSO_PIXEL_SHADER arg carries both the alpha-test tuple and the
  // dual-source flag, so emit it when either is active. Leaving it out
  // keeps the unspecialised PS shape for the common case.
  DXSO_SHADER_COMPILATION_ARGUMENT_DATA *head;
  bool emit_alpha = alpha_test_func != D3DCMP_ALWAYS;
  if (emit_alpha || dual_source) {
    ...
    alpha_arg.dual_source_blending = dual_source ? 1u : 0u;
