...
MTLD3D9Query::GetData(void *pData, DWORD dwSize, DWORD dwGetDataFlags) {
  D9DeviceLock lock = m_device->LockDevice();
  // Thin instrument wrapper: count the poll once at entry and the S_FALSE
  // (not-ready) return once at exit, so the six not-ready return paths below
  // need no per-site edits. The behavior is exactly getDataImpl's. The pair
  // separates an app that polls a ready query from one spinning on the GPU.
  auto *perf_stats = m_device->frameStats();
  dxmt::perf::addFrameCounter(perf_stats, &dxmt::FrameStatistics::frame_query_get_data_count);
  HRESULT hr = getDataImpl(pData, dwSize, dwGetDataFlags);
  if (hr == S_FALSE)
    dxmt::perf::addFrameCounter(perf_stats, &dxmt::FrameStatistics::frame_query_get_data_pending_count);
  return hr;
}
