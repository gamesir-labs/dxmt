...
// D3D9StateBlockChanges is defined in d3d9_device.hpp so the device's
// recording-time mask and the state-block's adopted mask share a type.
...
public:
  ...
  // Adopt the device's recording mask at EndStateBlock time. The
  // device collected per-state flags as Set* calls landed between
  // Begin/End; transferring those into the block lets Apply restore
  // exactly the touched states (matching wined3d's Apply shape).
  void
...
private:
  ...
  // m_renderStates).
  //
  // Full coverage of D3D9 state-block categories. Per-state-bit
  // masking (only restore states that were touched between Begin/End)
  // lands when the per-state-mask refactor does: same bookkeeping
