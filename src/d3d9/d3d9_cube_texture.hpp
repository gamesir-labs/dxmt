...
class MTLD3D9CubeTexture final : public ComObject<IDirect3DCubeTexture9>, public MTLD3D9CommonTexture {
public:
  ...
  // SYSTEMMEM/MANAGED cube's CPU-side mirror. Layout is face-major:
  // mirrorOffset(face, level) lives at index face*levelCount() + level.
  // m_mirror is a plain std::vector<uint8_t> (no Metal buffer wrapper),
  // so UpdateTexture routes through stageTextureUpload (CPU pointer +
  // staging-ring memcpy) rather than the buffer-direct path used for
  // 2D textures.
  const uint8_t *
  mirrorBase() const {
  ...
  // lazy-flag protocol UnlockRect uses.
  void
  flagAutoGenDirty() {
...
private:
  ...
  // See MTLD3D9Texture's m_mirror for the broader rationale; the
  // only delta is the 6× face dimension.
