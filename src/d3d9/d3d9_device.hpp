...
public:
  ...
  // Frame-statistics sink for a D3D9 entry point, or null when DXMT_PERF_STATS
  // is off. Taken from the queue, as the d3d11 immediate context does, rather
  // than from the perf thread-local: an entry point can run on any app thread,
  // and the thread-local only tracks the one that crossed the last present
  // boundary, so off-thread work would go uncounted.
  dxmt::FrameStatistics *
  frameStats() const {
    return dxmt::perf::enabled() ? &dxmtQueue().CurrentFrameStatistics() : nullptr;
  }
  // Charge the presenting thread's CPU and the fault counters since the
  // previous present to the frame that is ending. Called from the swapchain
  // Present tail, before the queue's present boundary closes the frame. The
  // carried-over sample assumes one presenting thread, which the D3D9 present
  // contract already implies; a second one would only mis-split its own frames.
  void recordPresentThreadCounters();
...
private:
  ...
  std::shared_ptr<dxmt::DeviceResidency> m_residency;
  ...
  // Previous present's cumulative counter sample. Only read and written by
  // recordPresentThreadCounters.
  WMTThreadCounters m_lastPresentCounters = {};
  bool m_havePresentCounters = false;
