...
public:
  ...
  // needed (audited M-PERF #3, M-RISK #13).
  struct BatchedDraw {
    D3D9DrawCapture cap;
    ...
    // non-owning (pinned by m_psoCache for device lifetime; mirrors d9vk).
    ...
    // coalescer (dxmt_context.cpp). Removing the per-draw
...
private:
  ...
  struct ResolveCache {
    ...
    // cached_pso_key_ shape (d3d9_device.cpp). Cheap memcmp,
...
private:
  ...
  // to 30 (DXVK) and accepts 0 as "use the default 3" (DXVK
  ...
  // (src/d3d11/d3d11_state_object.cpp). Without it, every draw in
