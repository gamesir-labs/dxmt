...
// SwapEffect, BackBufferCount (including the COPY single-backbuffer
// rule), and PresentationInterval. Multisampling is not validated here:
// the backbuffer is presented single-sampled regardless, so an MSAA
// request is accepted and downgraded rather than rejected. Pure read.
bool ValidatePresentParams(const D3DPRESENT_PARAMETERS &p, bool isEx);
