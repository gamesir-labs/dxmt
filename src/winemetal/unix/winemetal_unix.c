...
// Per-thread CPU plus process and host fault counters, sampled together so a
// caller can split a wall-clock interval into executing, faulting and blocked.
// The thread CPU comes from the mach thread rather than the Win32 thread times,
// which do not report this thread and can exceed the interval they cover. Runs
// on the caller's own OS thread, which is what makes mach_thread_self correct
// here.
NTSTATUS
_WMTQueryThreadCounters(void *obj) {
  struct unixcall_query_thread_counters *params = obj;
  memset(params, 0, sizeof(*params));

  mach_port_t thread = mach_thread_self();
  thread_basic_info_data_t thread_info_data;
  mach_msg_type_number_t thread_count = THREAD_BASIC_INFO_COUNT;
  if (thread_info(thread, THREAD_BASIC_INFO, (thread_info_t)&thread_info_data, &thread_count) == KERN_SUCCESS) {
    params->ret_thread_user_us =
        (uint64_t)thread_info_data.user_time.seconds * 1000000ull + thread_info_data.user_time.microseconds;
    params->ret_thread_system_us =
        (uint64_t)thread_info_data.system_time.seconds * 1000000ull + thread_info_data.system_time.microseconds;
    params->ret_valid |= 1;
  }
  mach_port_deallocate(mach_task_self(), thread);

  task_events_info_data_t events;
  mach_msg_type_number_t events_count = TASK_EVENTS_INFO_COUNT;
  if (task_info(mach_task_self(), TASK_EVENTS_INFO, (task_info_t)&events, &events_count) == KERN_SUCCESS) {
    params->ret_task_faults = events.faults;
    params->ret_task_pageins = events.pageins;
    params->ret_task_cow_faults = events.cow_faults;
    params->ret_valid |= 2;
  }

  // Host wide, so it counts other processes too. It is here for the zero fill
  // share within a burst, which is what separates commit churn from protection
  // faults; the absolute value is not attributable to this process.
  mach_port_t host = mach_host_self();
  vm_statistics64_data_t vm;
  mach_msg_type_number_t vm_count = HOST_VM_INFO64_COUNT;
  if (host_statistics64(host, HOST_VM_INFO64, (host_info64_t)&vm, &vm_count) == KERN_SUCCESS) {
    params->ret_vm_zero_fill_count = vm.zero_fill_count;
    params->ret_vm_reactivations = vm.reactivations;
    params->ret_valid |= 4;
  }
...
const void *__wine_unix_call_funcs[] = {
    ...
    // Index 156. Appended after the DXSO block so no existing index moves.
    &_WMTQueryThreadCounters,
};
...
const void *__wine_unix_call_wow64_funcs[] = {
    ...
    /* dxso_destroy_bitcode_params is sm50_ptr64_t-only, see DXSODestroy
       comment above. */
    &thunk_DXSODestroyBitcode,
    /* Index 156, lock-step with __wine_unix_call_funcs above.
       unixcall_query_thread_counters is uint64_t only, so the 64-bit
       implementation is byte-compatible: no thunk32 needed. */
    &_WMTQueryThreadCounters,
};
#endif
