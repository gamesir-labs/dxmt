...
// Sampled together on the calling thread: its own CPU split user/system, plus
// process and host fault counters. Lets a caller divide a wall-clock interval
// into executing, faulting and off-cpu, which no single interval can express.
struct WMTThreadCounters {
  uint64_t thread_user_us;
  uint64_t thread_system_us;
  uint64_t task_faults;
  uint64_t task_pageins;
  uint64_t task_cow_faults;
  uint64_t vm_zero_fill_count;
  uint64_t vm_reactivations;
  uint64_t valid;
};

WINEMETAL_API void WMTQueryThreadCounters(struct WMTThreadCounters *out);
